import{c3 as A,gK as j,c4 as E,c5 as R,fp as P,c6 as O,gL as u,fr as X,gM as V,gN as B,fs as H,c7 as D,c9 as s,ca as F,cb as T,cc as q,cd as m,ce as g,ch as o,fo as p}from"./main-46bfe858.js";const G=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Y={key:0},z={class:"flex justify-center"},J={key:0,class:"flex flex-col justify-center text-center"},Q={key:1,class:"flex flex-col justify-center"},U=["innerHTML"],W={key:3,class:"flex flex-col justify-center text-center"},Z=p("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1),ee={class:"text-xl mt-20"},te={key:1,class:"p-5"},ae=A({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(l){const a=l,r=j(),{t:d}=E(),h=R("iApi"),w=P(),f=O(()=>r.status),k=O(()=>r.response),C=P(!1),x=u({}),v=u([]),b=u([]);X(()=>{y(),v.push(h.event.on(V.LAYER_REMOVE,e=>{a.payload.layer?.uid===e.uid&&a.panel.close()})),b.push(B(()=>a.payload.layer.uid,(e,n)=>{e!==n&&y()}))}),H(()=>{v.forEach(e=>h.event.off(e)),b.forEach(e=>e())});const y=()=>{C.value=a.payload.layer!==void 0&&!a.payload.layer.isRemoved,a.payload.type==="xml"?N(a.payload.url,[]).then(e=>{r.status="success",e!==null&&w.value.childNodes[1].appendChild(e)}):a.payload.type==="html"&&S(a.payload.url).then(e=>{r.status=e.status,r.response=e.response})},N=(e,n)=>{let t=h.language==="en"?G:K;return t=t.replace(/\{\{([\w.]+)\}\}/g,(c,i)=>d(i)),x[e]?Promise.resolve(_(x[e],t,n)):S(e).then(c=>(x[e]=c.response,_(x[e],t,n)))},_=(e,n,t)=>{let c=null;if(window.XSLTProcessor){const i=new window.XSLTProcessor,L=new DOMParser,I=L.parseFromString(e,"text/xml"),M=L.parseFromString(n,"text/xml");i.importStylesheet(M),t&&t.forEach($=>i.setParameter("",$.key,$.value||"")),c=i.transformToFragment(I,document)}return c},S=e=>new Promise(n=>{const t=new XMLHttpRequest;t.open("GET",e,!0),t.responseType="text",t.onload=()=>{t.status===200?n({status:"success",response:t.response}):n({status:"error",response:"Could not load results from remote service."})},t.onerror=()=>{n({status:"error",response:"Could not load results from remote service."})},t.send()});return(e,n)=>{const t=D("panel-screen");return s(),F(t,{panel:l.panel,ref_key:"el",ref:w},{header:T(()=>[q(m(g(d)("metadata.title"))+": "+m(l.payload.layerName),1)]),content:T(()=>[C.value?(s(),o("div",Y,[p("div",z,[f.value=="loading"?(s(),o("div",J,m(g(d)("metadata.loading")),1)):l.payload.type==="xml"&&f.value=="success"?(s(),o("div",Q)):l.payload.type==="html"&&f.value=="success"?(s(),o("div",{key:2,innerHTML:k.value,class:"flex flex-col justify-center max-w-full"},null,8,U)):(s(),o("div",W,[Z,p("span",ee,m(g(d)("metadata.error")),1)]))])])):(s(),o("div",te,[p("span",null,m(g(d)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{ae as _};
