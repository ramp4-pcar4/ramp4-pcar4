...
For general or technical inquiries regarding the Reusable Accessible Mapping Platform that this interactive map is based on, please contact <applicationsdecartographieweb-webmappingapplications@ec.gc.ca>. You may access the RAMP GitHub repository using the "About RAMP" button in the lower-left corner. 
...
Access the menu by clicking on the menu button near the top left of the viewer.
...
The Layers panel, accessed from the first option in the left menu, serves as a map legend and lists the layers available to display in the map.
...
Some layers may only be visible at certain zoom levels. If a layer is not visible at a given zoom level the legend will display an icon indicating this status. Further, a zoom mangifying glass icon to the nearest valid level button will be displayed. 
...
Select the "basemaps" icon in the left-hand menu. You will be presented with one or more basemaps to choose from, separated by their projection types (Lambert vs. Web Mercator). The map will reload if you change projections, but not if you switch basemaps within the same projection.
