...
- Each layer type allows the choice of a **Layer Name**, which will be displayed in the legend.
...
- A WFS Service or a file based dataset allows the choice of a **Primary Field**, which acts the same as in a Feature Layer; a **Colour**, which determines the colour of the points / lines / polygons on the map. File based datasets also allow the choice of a **Tooltip Field** which acts the same as in a Feature Layer. CSV files also allow the specification of the columns that contain the Latitude and Longitude values, used to derive the point location on the map. All other information is derived from the source's metadata.
