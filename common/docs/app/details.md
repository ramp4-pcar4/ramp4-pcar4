...
Template bindings should be placed in the `fixtures.details.items` section of the configuration file. When the details panel first loads, it looks for any template bindings in here. The `id` should match the layer `id`, and the `template` should match the name of the custom component that you registered on the host page.
