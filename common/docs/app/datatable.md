...
### The Grid
The grid is the main component of the datatable. It contains a row for each data point on the layer. The grid columns correspond to the attributes that belong to the data points.

### Global and Column Filters
Global and column filters allow the user to display smaller subsets of data within the table. See the [global filter](#Global-Filter) and [column filters](#Column-Filters).

### Record Counter
The record counter is located in the upper left corner of the datatable. It displays the range of rows that are currently visible on the table, and the total number of records that are in the table. If any filters are applied to the grid, the number of filtered rows will be displayed instead of the total number.

### Zoom and Details Buttons
Each row in the datatable contains a zoom and details button. 

**TODO:** update this once these features are implemented.
...
## Global Filter
The global filter is located in the panel header. It is a text filter that applies to *all columns* in the table. It will display any row with an entry that matches the entered text.
