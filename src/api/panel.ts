...
export class PanelAPI extends APIScope {
    ...
    updateHTML(
        ...
        if (isHTMLScreen(screen)) {
            screen.en.innerHTML =
                html.en instanceof HTMLElement ? html.en.outerHTML : html.en;

            screen.fr.innerHTML =
                html.fr instanceof HTMLElement ? html.fr.outerHTML : html.fr;
        } else {
    ...
    private registerHTMLConfig(
        ...
        if (typeof html.en === 'string') {
            const newHtmlEn = document.createElement('div');
            newHtmlEn.innerHTML = html.en;
            html.en = newHtmlEn;
        }
        if (typeof html.fr === 'string') {
            const newHtmlFr = document.createElement('div');
            newHtmlFr.innerHTML = html.fr;
            html.fr = newHtmlFr;
        }
    ...
    registerHTML(
        ...
        this.$iApi.event.on(GlobalEvents.LANG_CHANGE, () => {
            this.isRegistered(id).then(() => {
                const currPanel = this.get(id);
                const panelOpen = currPanel.isOpen;
                const panelConfig = this.registerHTMLConfig(
                    currPanel.screens[id],
                    id,
                    alertName,
                    style
                );
                ...
                const newPanel: PanelInstance = this.register(
                    panelConfig,
                    options
                ) as unknown as PanelInstance;

                // if the html panel was closed, it shouldnt be opened automatically upon config change
                if (panelOpen) {
                    this.open(newPanel);
                }
            });
