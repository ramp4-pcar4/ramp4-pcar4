...
export class MapAPI extends CommonMapAPI {
    ...
    async addLayer(
        ...
        if (
            layer.layerState !== LayerState.INITIATING &&
            layer.layerState !== LayerState.INITIATED &&
            layer.loadState !== LoadState.ERROR
        ) {
        ...
        const layerWatcher = setInterval(() => {
            timeElapsed += 1000;
            if (timeElapsed >= 600000 || layer.loadState === LoadState.ERROR) {
                // Do we want to set this to something else?
                this.$iApi.$vApp.$store.set(LayerStore.addErrorLayers, [layer]);
                layer.updateLoadState(LoadState.ERROR); // need this thanks to an edge case where the legend sometimes doesnt update
