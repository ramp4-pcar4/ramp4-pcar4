 ...
 */
export class FlatGeobufLayer extends FileLayer {
    ...
    protected async onInitiate(): Promise<void> {
        // get fgb from appropriate source, convert to geojson and set to special property.
        // then initiate the FileLayer

        const startTime = Date.now();

        const fgbData = await this.$iApi.geo.layer.files.binaryInitHelper(
            this.origRampConfig
        );
