...
export class CommonLayer extends LayerInstance {
    ...
    protected constructor(rampConfig: RampLayerConfig, $iApi: InstanceAPI) {
        ...
        this._drawOrder = [];
        if (
            typeof rampConfig.expectedDrawTime === 'undefined' ||
            rampConfig.expectedDrawTime > 0
        ) {
            this.expectedTime.draw = rampConfig.expectedDrawTime ?? 4000;
        }
        if (
            typeof rampConfig.expectedResponseTime === 'undefined' ||
            rampConfig.expectedResponseTime > 0
        ) {
    ...
    protected startTimer(type: TimerType): void {
        if (this.timers[type]) this.stopTimer(type); // reset the timer if a timing is already in progress.
        if (this.expectedTime[type]) {
