...
const detailsTemplate = computed(() => {
    ...
    if (layer && detailProperties.value[layer.id] && detailProperties.value[layer.id].template) {
        const customTemplateExists =
            typeof resolveDynamicComponent(detailProperties.value[layer.id].template) !== 'string';
        ...
        if (customTemplateExists) {
            ...
            console.error(
                `Could not find custom details template named ${detailProperties.value[layer.id].template}. Was it registered correctly?`
            );
        }
    }
