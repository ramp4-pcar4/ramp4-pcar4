...
export default defineComponent({
    ...
    data() {
        return {
            ...
            greedyLoading: false as boolean,
            abortFlag: false as boolean,
            lastIdentifyTime: 0 as number,
    ...
    methods: {
        ...
        loadPayloadItems(newPayload: Array<IdentifyResult>): void {
            ...
            // track last identify timestamp and add to payload items
            this.lastIdentifyTime = Date.now();
            newPayload.forEach(
                (item: IdentifyResult) =>
                    (item.lastTime = this.lastIdentifyTime)
            );
            ...
            if (this.lastLayerUid && itemsPanel.isOpen) {
                ...
                if (lastIdx !== -1) {
                    ...
                    lastIdentify.loading.then(() => {
                        // new identify request came in while loading old results, turn abort flag on
                        if (lastIdentify.lastTime !== this.lastIdentifyTime) {
                            this.abortFlag = true;
                        }
        ...
        autoOpenAny(newPayload: Array<IdentifyResult>): void {
            ...
            Promise.any(loadingResults)
                .then((res: IdentifyResult) => {
                    // new identify request came in while loading old results, turn abort flag on
                    if (res.lastTime !== this.lastIdentifyTime) {
                        this.abortFlag = true;
                    }
         ...
         */
        closeResult(): void {
            // skip close panel command if abort flag is on
            if (this.abortFlag) {
                return;
            }
        ...
        openResult(index: number) {
            // skip open panel command if abort flag is on
            if (this.abortFlag) {
                return;
            }

            if (this.payload[index].items.length > 0) {
                // turn abort flag on if user clicks item before greedy open is finished
                if (this.greedyLoading) {
                    this.abortFlag = true;
                }
