...
export default defineComponent({
    ...
    data() {
        return {
            ...
            lastIdentifyTime: undefined as number | undefined,
    ...
    beforeUnmount() {
        ...
        this.$iApi.event.off(GlobalEvents.DETAILS_CLOSED);
    },
    methods: {
        /**
        ...
        loadPayloadItems(newPayload: Array<IdentifyResult>): void {
            ...
            this.lastIdentifyTime = newPayload[0].requestTime;
            ...
            if (this.lastLayerUid && itemsPanel.isOpen) {
                ...
                if (lastIdx !== -1) {
                    ...
                    lastIdentify.loading.then(() => {
                        ...
                        if (
                            lastIdentify.requestTime !== this.lastIdentifyTime
                        ) {
                            return;
                        }
        ...
        autoOpenAny(newPayload: Array<IdentifyResult>): void {
            ...
            Promise.any(loadingResults)
                .then((res: IdentifyResult) => {
                    // new identify request came in while loading old results, exit greedy algo
                    if (res.requestTime !== this.lastIdentifyTime) {
                        return;
                    }
                ...
                .catch(() => {
                    // new identify request came in while loading old results, exit greedy algo
                    if (lastTime !== this.lastIdentifyTime) {
                        return;
                    }
