<template>
    ...
    >
        ...
        <span v-else v-html="getZoomIcon(iApi.ui.zoomIcon)"></span>
    </button>
</template>
...
const mapLayer = ref<boolean | undefined>(undefined);
...
onBeforeMount(() => {
    // Determine whether the layer this button is associated actually has map data. If not,
    // don't show the zoom button.
    mapLayer.value = layerStore.getLayerByUid(
        props.params.data.rvUid
    )?.mapLayer;
});
