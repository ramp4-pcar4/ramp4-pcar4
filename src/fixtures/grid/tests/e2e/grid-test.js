const toggleGrid = legendName => {
    ...
    cy.contains('.legend-item', legendName).find('.progress-line').should('not.exist');
...
describe('Grid', () => {
                ...
                .then(attributes => {
                    ...
                    cy.get('.ag-center-cols-viewport .ag-row').each($row => {
                        ...
                        for (attrib in attributes.features[idx]) {
                            ...
                            cy.wrap($row).find(`.ag-cell[col-id="${attrib}"]`).contains(val);
                        }
                    });
                });
            toggleGrid('Clean Air');
        ...
        it('uses cache', () => {
            ...
            cy.window()
                ...
                .then(p => {
                    ...
                    expect(p.getEntriesByName('cache')[0].duration).to.be.lessThan(
                ...
                .then(layer => {
                    cy.wrap(layer)
                        ...
                        .then(attributes => {
                            ...
                            cy.get('.ag-center-cols-viewport .ag-row').each($row => {
                                // get oid from row
                                const oid = Object.keys(attributes.oidIndex).find(
                                    key => attributes.oidIndex[key] === parseInt($row.attr('row-id'))
                                );
                ...
                cy.get(`.ag-center-cols-container [row-id="${row}"]`)
                    ...
                    .then($oid => {
                        cy.window().then(window => {
                            ...
                            cy.wrap(window.debugInstance.$vApp.$store.get('layer/getLayerById', 'CleanAir'))
                                .invoke('getGraphic', $oid.text(), {
                                    getGeom: true
                                })
                                .then(g => {
                                    const center = window.debugInstance.geo.map.getExtent().center();
    ...
    describe('Header', () => {
        ...
        it('reorders columns', () => {
            ...
            cy.get('.ag-center-cols-container .ag-row:first [col-id="Name"] .ag-cell-value').should(
                'have.attr',
                'aria-colindex',
                4
            );
            ...
            cy.get('.ag-center-cols-container .ag-row:first [col-id="OBJECTID"] .ag-cell-value').should(
                'have.attr',
                'aria-colindex',
                9
            );
            // click right reorder arrow, should still be 9th col
            cy.get('.ag-header-row [col-id="OBJECTID"] #keyboard_arrow_right').click({
                force: true
            });
            cy.get('.ag-center-cols-container .ag-row:first [col-id="OBJECTID"] .ag-cell-value').should(
                'have.attr',
            ...
            cy.get('.ag-header-row [col-id="OBJECTID"] #keyboard_arrow_left').click({
                force: true
            });
            cy.get('.ag-center-cols-container .ag-row:first [col-id="OBJECTID"] .ag-cell-value').should(
                'have.attr',
    ...
    describe('Filters', () => {
        ...
        it('filters numbers both bounds', () => {
            ...
            cy.get('.ag-center-cols-container .ag-cell[col-id="OBJECTID"]').each($cell => {
                cy.wrap(parseInt($cell.text().trim())).should('gte', 4848).and('lte', 4860);
            });
        ...
        it('filters numbers single value', () => {
            ...
            cy.get('.ag-center-cols-container .ag-cell[col-id="OBJECTID"]').should().should('have.length', 1);
        ...
        it('filters numbers both bounds with tab support', () => {
            toggleGrid('Carbon monoxide');
            // filter "OBJECTID" field
            cy.get('[aria-rowindex="2"] [aria-colindex="4"] .rv-min').type('4848').tab().type('4860').blur();
        ...
        it('filters date start date only', () => {
            ...
            cy.get('[aria-rowindex="2"] [aria-colindex="7"] .rv-input').eq(0).type('2003-05-07').blur();
        ...
        it('filters date single date value', () => {
            ...
            cy.get('[aria-rowindex="2"] [aria-colindex="7"] .rv-input').eq(0).type('2000-12-05').blur();
            cy.get('[aria-rowindex="2"] [aria-colindex="7"] .rv-input').eq(1).type('2000-12-05').blur();
            ...
            cy.get('.ag-center-cols-container .ag-cell[col-id="risc_date"]').should().should('have.length', 3);
        ...
        it('filters multiple columns', () => {
            ...
            cy.get('[aria-rowindex="2"] [aria-colindex="6"] .rv-input').type('plant', {
                force: true
            });
            cy.get('[aria-rowindex="2"] [aria-colindex="9"] .rv-input').type('toronto', {
                force: true
            });
        ...
        it('filters multiple columns with global field', () => {
            ...
            // need to force because not all fields will be visible
            cy.get('[aria-rowindex="2"] [aria-colindex="6"] .rv-input').type('prince', {
                force: true
            });
            cy.get('[aria-rowindex="2"] [aria-colindex="9"] .rv-input').type('prince', {
                force: true
            });
