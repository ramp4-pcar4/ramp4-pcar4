...
const gridLayers = computed(() => {
    if (gridStore.grids[props.gridId]) {
        const layerIds = gridStore.grids[props.gridId].layerIds;
        ...
        const modifiable = layerIds.map(id => {
        ...
        return layerIds.map(id => iApi.geo.layer.getLayer(id) as LayerInstance);
    } else return [];
});
