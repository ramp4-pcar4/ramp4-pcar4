    ...
    Promise.all(
        gridLayers.value.map(async layer => {
            if (layer && layer.visibility) {
                // since json data layers don't support filtering, and things will explode if the code below runs,
...
const cancelAttributeLoad = () => {
    if (isLoadingGrid.value || isErrorGrid.value) {
        ...
        gridLayers.value
            .filter(layer => layer.layerType !== LayerType.DATAJSON)
            .forEach(layer => {
    ...
    Promise.all(fancyLayers.map(l => l.loadPromise())).then(() => {
        ...
        Promise.all(tableAttributePromises)
            .then((tableAttributes: Array<TabularAttributeSet>) => {
                ...
                if (
                    fancyLayers
                        .filter(fl => fl.layerType !== LayerType.DATAJSON)
                        .every(fl => fl.attribLoadAborted())
                ) {
