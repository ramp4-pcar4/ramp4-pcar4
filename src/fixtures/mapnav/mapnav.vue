<template>
    <div class="mapnav absolute right-0 bottom-0 pb-36 sm:pb-48 pr-12">
        ...
        >
            ...
            <div v-if="windowHeight > 460" class="mapnav-section bg-white-75 hover:bg-white">
        ...
        <div
            v-if="windowHeight <= 460 && sepNavVisible"
            class="absolute right-56 bottom-36 sm:bottom-48 mapnav-section flex flex-col flex-wrap gap-x-1 items-start"
            :class="{ 'h-[165px]': windowHeight <= 400, 'h-[99px]': windowHeight <= 340 }"
            v-focus-list
            :content="t('panels.controls.items')"
            v-tippy="{
                trigger: 'manual',
        ...
        >
            <template v-for="(button, index) in visible" :key="button.id + 'button'">
                <component :is="button.id + '-nav-button'" class="mapnav-section bg-white-75 hover:bg-white" />
                <divider-nav class="mapnav-divider" v-if="index !== visible.length - 1" />
            </template>
        </div>
    </div>
...
import Button from '@/fixtures/mapnav/button.vue';
