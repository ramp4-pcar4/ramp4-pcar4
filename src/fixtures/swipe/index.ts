...
class SwipeFixture extends FixtureInstance {
    ...
    async added(): Promise<void> {
        ...
        this.addTrailingLayers = (layerIds: Array<string>): void => {
            layerIds.forEach(layerId => {
                const layer = this.$iApi.geo.layer.getLayer(layerId)?.esriLayer;
                if (layer) swipe.trailingLayers.add(layer);
            });
            swipe.trailingLayers = trailingLayers;
        };

        this.addLeadingLayers = (layerIds: Array<string>): void => {
            layerIds.forEach(layerId => {
                const layer = this.$iApi.geo.layer.getLayer(layerId)?.esriLayer;
                if (layer) swipe.leadingLayers.add(layer);
            });
        };
    }
}
