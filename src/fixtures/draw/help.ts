...
- Details: view the shape type, measurements, feature counts, centroid, extent, segments, and vertices. Use ${actionButton('Run Identify', DRAW_HELP_ICONS.identify)} to refresh feature counts, ${actionButton('Copy', DRAW_HELP_ICONS.copy)} coordinate or segment values, or ${actionButton('Export', DRAW_HELP_ICONS.download)} the selected shape.
...
- Détails : affichez le type de forme, les mesures, les dénombrements d'entités, le centroïde, l'étendue, les segments et les sommets. Utilisez ${actionButton("Exécuter l'identification", DRAW_HELP_ICONS.identify)} pour actualiser les dénombrements d'entités, ${actionButton('Copier', DRAW_HELP_ICONS.copy)} des coordonnées ou des valeurs de segment, ou ${actionButton('Exporter', DRAW_HELP_ICONS.download)} la forme sélectionnée.
