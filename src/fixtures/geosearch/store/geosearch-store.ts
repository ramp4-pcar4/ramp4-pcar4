...
export const useGeosearchStore = defineStore('geosearch', () => {
        ...
        () =>
            new Promise(resolve => {
                GSservice.value.fetchTypes().then((types: Array<any>) => {
                    const customTypes = customSources[appLang.value].value.map((src: ICustomSource) => ({
                        code: src.code,
                        name: src.catName
                    }));
    ...
    function initService(lang: 'en' | 'fr', config: any) {
        appLang.value = lang;
        ...
        if (config?.customSources) {
            config.customSources.forEach((source: any) => {
                if (!customSources[lang].value.some((s: ICustomSource) => s.code === source.code)) {
                ...
                const watcher = setInterval(() => {
                    if (GSservice.value.config.provinces.listFetched && GSservice.value.config.types.typesFetched) {
                        ...
                        GSservice.value.query(`${cleanedSearchVal}*`).then((data: any) => {
                            ...
                            Promise.all(
                                customSources[appLang.value].value.map((src: ICustomSource) => src.onSearch(cleanedSearchVal))
