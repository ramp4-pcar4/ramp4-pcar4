import{dM as H,fa as A,dN as R,e2 as T,dQ as _,gd as C,e7 as j,e8 as V,dE as X,e6 as U,e9 as F,dS as q,dT as n,dW as k,dX as d,d_ as p,e1 as c,dV as h,eb as D,dU as B}from"./main-BenRwYBM.js";import{m as G}from"./marked.esm-DgqJBp_S.js";const Q=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Y={key:0},z={class:"flex justify-center"},J={key:0,class:"flex flex-col justify-center text-center"},K=["innerHTML"],Z=["innerHTML"],ee={key:3,class:"flex flex-col justify-center text-center"},te={class:"text-xl mt-20"},ae={key:1,class:"p-5"},le=H({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(r){const s=A(),{t:o}=R(),u=V("iApi"),M=T(),l=r,f=_(()=>s.status),v=_(()=>s.response),b=T(!1),x=C({}),y=C([]),$=C([]);j(()=>{S(),y.push(u.event.on(X.LAYER_REMOVE,e=>{l.payload.layer?.uid===e.uid&&l.panel.close()})),$.push(U(()=>l.payload.layer.uid,(e,t)=>{e!==t&&S()}))}),F(()=>{y.forEach(e=>u.event.off(e)),$.forEach(e=>e())});const S=()=>{b.value=l.payload.layer!==void 0&&!l.payload.layer.isRemoved,l.payload.type==="xml"?I(l.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(g(`${e.firstElementChild.outerHTML}`)),(l.payload.catalogueUrl||l.payload.url)&&t.appendChild(g(`<h5 class="text-xl font-bold mb-3">${o("metadata.xslt.metadata")}</h5>`)),l.payload.catalogueUrl&&t.appendChild(g(`<p><a style="color: blue;" href="${l.payload.catalogueUrl}" target="_blank">${o("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(g(`<p><a style="color: blue;" href="${l.payload.url}" target="_blank">${o("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):l.payload.type==="html"?w(l.payload.url).then(e=>{s.status=e.status,s.response=e.response}):l.payload.type==="md"&&w(l.payload.url).then(e=>{s.status=e.status,s.response=G(e.response)})},I=(e,t)=>{let a=u.language==="en"?Q:W;return a=a.replace(/\{\{([\w.]+)\}\}/g,(i,m)=>o(m)),x[e]?Promise.resolve(L(x[e],a,t)):w(e).then(i=>(x[e]=i.response,L(x[e],a,t)))},L=(e,t,a)=>{let i=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,O=new DOMParser,N=O.parseFromString(e,"text/xml"),E=O.parseFromString(t,"text/xml");m.importStylesheet(E),a&&a.forEach(P=>m.setParameter("",P.key,P.value||"")),i=m.transformToFragment(N,document)}return i},w=e=>new Promise(t=>{const a=new XMLHttpRequest;a.open("GET",e,!0),a.responseType="text",a.onload=()=>{a.status===200?t({status:"success",response:a.response}):t({status:"error",response:"Could not load results from remote service."})},a.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},a.send()});function g(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const a=B("panel-screen");return n(),q(a,{panel:r.panel,ref_key:"el",ref:M},{header:k(()=>[D(c(h(o)("metadata.title"))+": "+c(r.payload.layerName),1)]),content:k(()=>[b.value?(n(),d("div",Y,[p("div",z,[f.value=="loading"?(n(),d("div",J,c(h(o)("metadata.loading")),1)):r.payload.type==="xml"&&f.value=="success"?(n(),d("div",{key:1,innerHTML:v.value,class:"flex flex-col justify-center xml-content"},null,8,K)):(r.payload.type==="html"||r.payload.type==="md")&&f.value=="success"?(n(),d("div",{key:2,innerHTML:v.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,Z)):(n(),d("div",ee,[t[0]||(t[0]=p("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),p("span",te,c(h(o)("metadata.error")),1)]))])])):(n(),d("div",ae,[p("span",null,c(h(o)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{le as default};
