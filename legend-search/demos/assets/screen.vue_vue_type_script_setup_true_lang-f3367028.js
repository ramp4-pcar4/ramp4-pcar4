import{gK as j,ib as A,gL as R,gM as X,gY as $,gN as O,hB as w,gZ as E,hj as V,hv as B,g_ as H,gO as D,gQ as o,gR as F,gS as T,hp as q,h6 as x,gP as s,gX as l,gT as p}from"./main-95e6e76d.js";const G=`<?xml version="1.0" encoding="UTF-8"?>
...
`,K={key:0},Q={class:"flex justify-center"},Z={key:0,class:"flex flex-col justify-center text-center"},z={key:1,class:"flex flex-col justify-center"},J=["innerHTML"],U={key:3,class:"flex flex-col justify-center text-center"},W=p("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1),ee={class:"text-xl mt-20"},te={key:1,class:"p-5"},ae=j({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(r){const a=r,d=A(),{t:i}=R(),h=X("iApi"),u=$(),f=O(()=>d.status),k=O(()=>d.response),C=$(!1),g=w({}),b=w([]),v=w([]);E(()=>{y(),b.push(h.event.on(V.LAYER_REMOVE,e=>{a.payload.layer?.uid===e.uid&&a.panel.close()})),v.push(B(()=>a.payload.layer.uid,(e,n)=>{e!==n&&y()}))}),H(()=>{b.forEach(e=>h.event.off(e)),v.forEach(e=>e())});const y=()=>{C.value=a.payload.layer!==void 0&&!a.payload.layer.isRemoved,a.payload.type==="xml"?N(a.payload.url,[]).then(e=>{d.status="success",e!==null&&u.value.childNodes[1].appendChild(e)}):a.payload.type==="html"&&S(a.payload.url).then(e=>{d.status=e.status,d.response=e.response})},N=(e,n)=>{let t=h.language==="en"?G:Y;return t=t.replace(/\{\{([\w.]+)\}\}/g,(c,m)=>i(m)),g[e]?Promise.resolve(_(g[e],t,n)):S(e).then(c=>(g[e]=c.response,_(g[e],t,n)))},_=(e,n,t)=>{let c=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,L=new DOMParser,I=L.parseFromString(e,"text/xml"),M=L.parseFromString(n,"text/xml");m.importStylesheet(M),t&&t.forEach(P=>m.setParameter("",P.key,P.value||"")),c=m.transformToFragment(I,document)}return c},S=e=>new Promise(n=>{const t=new XMLHttpRequest;t.open("GET",e,!0),t.responseType="text",t.onload=()=>{t.status===200?n({status:"success",response:t.response}):n({status:"error",response:"Could not load results from remote service."})},t.onerror=()=>{n({status:"error",response:"Could not load results from remote service."})},t.send()});return(e,n)=>{const t=D("panel-screen");return o(),F(t,{panel:r.panel,ref_key:"el",ref:u},{header:T(()=>[q(x(s(i)("metadata.title"))+": "+x(r.payload.layerName),1)]),content:T(()=>[C.value?(o(),l("div",K,[p("div",Q,[s(f)=="loading"?(o(),l("div",Z,x(s(i)("metadata.loading")),1)):r.payload.type==="xml"&&s(f)=="success"?(o(),l("div",z)):r.payload.type==="html"&&s(f)=="success"?(o(),l("div",{key:2,innerHTML:s(k),class:"flex flex-col justify-center max-w-full"},null,8,J)):(o(),l("div",U,[W,p("span",ee,x(s(i)("metadata.error")),1)]))])])):(o(),l("div",te,[p("span",null,x(s(i)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{ae as _};
