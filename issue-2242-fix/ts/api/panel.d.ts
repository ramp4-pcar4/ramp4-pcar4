...
export declare class PanelAPI extends APIScope {
    panelStore: import("pinia").Store<"panel", import("pinia")._UnwrapAll<Pick<import("@/stores/panel").PanelStore, "items" | "visible" | "reorderable" | "pinned" | "priority" | "stackWidth" | "remWidth" | "mobileView" | "regPromises" | "orderedItems" | "teleported">>, Pick<import("@/stores/panel").PanelStore, "getRemainingWidth">, Pick<import("@/stores/panel").PanelStore, "open" | "close" | "move" | "remove" | "getVisible" | "getRegPromises" | "openPanel" | "closePanel" | "movePanel" | "removePanel" | "setStackWidth" | "setMobileView" | "updateVisible" | "registerPanel" | "addRegPromise">>;
    /** Updates the content of a specific screen of a panel, using HTML content
     *
     ...
     * @param {string} [id] id of the screen to be updated. If not provided, it is assumed that there is only one screen in the panel.
     * @memberof PanelAPI
     */
    updateHTML(panel: PanelInstance, html: string | HTMLElement, id?: string): void;
    /** Registers HTML content as a `PanelInstance` object and returns it.
     * When the panel is registered, all its screens are added to the Vue as components right away.
     *
     * @param {string | HTMLElement} html HTML content, represented as an HTMLElement object or a string.
     * @param {string} id Used as the id of the PanelConfig object passed to `register()` as well as the id of the (individual) screen registered for the panel.
     * @param {PanelConfigStyle} [styles] The styles to apply to the panel. If none provided, the default panel styling will be used.
     * @param {string} [alertName] Translation key (or string) to use in panel screen reader alerts. If none provided, a default will be used.
     ...
     * @memberof PanelAPI
     */
    registerHTML(html: string | HTMLElement, id: string, style?: PanelConfigStyle, alertName?: string, options?: PanelRegistrationOptions): PanelInstance;
    /**
 ...
 */
export declare type PanelConfigSet = {
    [name: string]: PanelConfig;
};
/**
 * A single pair of `PanelConfig`/id values.
 */
export declare type PanelConfigPair = {
    id: string;
    config: PanelConfig;
};
/**
 ...
 */
export declare type PanelInstanceSet = {
 ...
 */
export declare type PanelInstancePath = {
    id: string;
    screen?: string;
    props?: object;
};
 ...
 */
export declare type PanelRegistrationOptions = {
    /**
...
export declare type PanelTeleportConfig = PanelTeleportObject | {
