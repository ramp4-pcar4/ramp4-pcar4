...
The legend fixture contains a panel which displays all layers which have been added to the map. The legend panel allows for easy access to each layer's data table (if applicable), settings, and metadata (if applicable). The panel also allows you to refresh and delete layers if they are configured to allow it.

The legend fixture is a default fixture, meaning it is automatically added to RAMP and loaded upon start up. 
...
#### Legend Entry
A legend entry represents a single layer on the map. A layer's legend entry allows you to interact with the layer. Each legend entry has a check box that allows you to toggle the layer's visibility on or off. If supported, clicking on the legend entry will open the data table associated with the layer. Legend entries also contain a `More options` button, which contains more interactive options:
- Metadata: *if a metadata URL is provided, opens a panel displaying the data.*
- Settings: *opens a settings panel that allows you to control layer opacity, visibility, and more.*
- Datatable: *opens the data table for the associated layer. Same action as simply clicking on the legend entry.*
- Legend: *expands or collapses the layer symbology stack.*
- Zoom to Layer Boundary: *sets the map zoom level to contain all layer features.*
- Remove: *removes the layer from the legend and map.*
- Reload: *reloads the layer.*

These controls can be disabled for a specific layer through its object in the legend portion of the configuration file. To do this, add the `disbledControls` property to the object as an array. The following example demonstrates disabling the boundary zoom for a layer called `CleanAir`:
...
#### Legend Group
A legend group is made up of one or more legend entries. A legend group can be collapsed to hide the legend entires contained within it. Toggling the visibility on the group, if enabled, will toggle the visibility of all of the legend entries contained within.

#### Visibility Group
A visibility group works similar to the legend group, however the legend only allows for one child of the group to the visible at a time. If one child is visible and you toggle the visibility on for another child, the visibility for the first child will be turned off.
...
- `isOpen: boolean`, true if the legend panel is open by default.
- `isPinned: boolean`, true if the legend panel is pinned by default.
...
### Layer Objects
There are four types of layer objects for the legend fixture, each one representing one of the [Components](#Components) mentioned earlier in this documentation.
...
`collapse`: a boolean, if true renders the visibility set as a single legend entry.
...
### Read
Provides the following methods:
- `getLegend()`: returns the full legend tree.
...
- `reloadLayerItem(layerId: string)`: reloads the layer with the id provided.
