...
* (readonly) `id` - a string representing the ID of the panel.
* (readonly) `screens` - an object representing the screens to be displayed inside the panel. Each key represents a screen ID, while each value represents the screen itself, which must be one of the following types:
...
* (readonly) `loadedScreens` - a list of strings, where each string is a screen ID (a key in the `screens` object), representing the screens that are ready to be rendered on the page. If a screen is not ready to be rendered, a loading spinner is shown instead of the screen's contents.
* (readonly) `alertName` - the i18n key for what the panel should be referred to when using RAMP with screen readers. Please see the [localization documentation](../configuration/config-language.md) for details.
...
* (readonly) `expanded` - a boolean representing whether the panel is currently expanded to take up the full width of the stack.
* (readonly) `isLeftMostPanel` - a boolean representing whether the panel is the leftmost in the stack.
* (readonly) `isRightMostPanel` - a boolean representing whether the panel is the rightmost in the stack.
* (readonly) `isOpen` - a boolean representing whether the panel is currently open.
* (readonly) `isPinned` - a boolean representing whether the panel is currently pinned.
* (readonly) `isVisible` - a boolean representing whether the panel is currently visible.
* (readonly) `width` - the width of the panel.
