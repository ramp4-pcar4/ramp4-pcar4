...
* when a panel is opened while there is no more space, the first panel on the left that is not pinned is hidden, but not fully closed.
...
*  pin - pins the panel in place, which prevents the panel from being closed when the stack runs out of space. Therefore, if the leftmost panel is pinned and the stack runs out of space, the pinned panel stays visible and the next most leftmost panel is closed.
* left/right - moves the panel one index to the left/right in the stack.
...
* `id` - a string representing the ID of the panel.
* `screens` - an object representing the screens to be displayed inside the panel. Each key represents a screen ID, while each value represents the screen itself, which must be one of the following types:
    ...
    More information about the above options can be found in the [Vue documentation](https://vuejs.org/guide/introduction.html).
* `loadedScreens` - a list of strings, where each string is a screen ID, representing the screens that are ready to be rendered on the page.
* `alertName` - the ii8n key for what the panel should be referred to when using RAMP with screen readers.
* `controls` - an object with string keys representing a control and boolean values representing whether the control is to be enabled or not. Note that this does not affect default controls such as close, minimize, left/right, and back. However, the expand control is affected.
* `button` - an object representing the appbar button for this panel. It has the following properties:
    ...
    - `tooltip` - the ii8n key for the text to display when hovering over the button.
* `route` - an object that represents the panel screen to be displayed for the panel. It has the following properties:
    - `screen` - the screen ID to display.
...
* `expanded` - a boolean representing whether the panel is currently expanded to take up the full width of the stack.
* `isLeftMostPanel` - a boolean representing whether the panel is the leftmost in the stack.
* `isRightMostPanel` - a boolean representing whether the panel is the rightmost in the stack.
* `isOpen` - a boolean representing whether the panel is currently open.
* `isPinned` - a boolean representing whether the panel is currently pinned.
* `isVisible` - a boolean representing whether the panel is currently visible.
* `width` - the width of the panel.

Additionally, the `PanelInstance` Object has the following methods:
* `registerScreen(id: string)` - loads panel screen components for the specified screen. This should be called just before the screen is to be shown.
...
* `move(direction: string)` - moves the panel by one unit in the specified direction (left or right), if possible.
* `remove()` - removes the panel from the stack.
* `toggle(value?: boolean | { screen: string; props?: object; toggle?: boolean })` - toggle panel if no value provided, force toggle panel if value specified, or toggle panel on specified screen if provided.
...
* `show(value: string | { screen: string; props?: object })` - sets route (panel to be shown) to the specified screen id and pass props to the panel screen components.
...
*  `open` - an array of objects that describes which panels to open on startup. Each object has the following properties:
    - `id` - the ID of the panel to open. The following stock panel ID's are currently supported: `basemap`, `export`, `geosearch`, `help`, `layer-reorder`, `legend`, `notifications`, and `wizard`.
    ...
    Note that only panels without props can currently be opened on startup via the config.
...
Your panel is now ready for use! You should now leverage the [API](#the-panel-api) to perform different functions of your choice. For example, you can display the panel on the screen with the `open` method, pin the panel on the screem with the `pin` method, and so on.
...
* `move(value: string | PanelInstance, direction: string): PanelInstance` - moves the specified panel one unit in the specified direction, if possible.
* `toggle(value: string | PanelInstance | PanelInstancePath, toggle?: boolean): PanelInstance` - toggles the panel. The optional `toggle` boolean can be used to force the panel to open/close (`true` for open, `false` for close).
