...
* `alertName` - the i18n key for what the panel should be referred to when using RAMP with screen readers.
...
* `button` - an object representing the appbar button for this panel. It has the following properties:
    ...
    - `tooltip` - the i18n key for the text to display when hovering over the button.
