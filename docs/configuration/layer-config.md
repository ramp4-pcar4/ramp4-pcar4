...
- File type layers will point to a web hosted file, or an empty string if the `rawData` property is provided.
...
Defines what controls should be available for this layer in the app. This list acts as the "layer default", but controls on specifc fixtures can be overridden by configuration in the `fixtures` object of the layer configuration.
