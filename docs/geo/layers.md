...
In general, most layer properties and methods should only be accessed after the layer has loaded. Attempts to use prior to that may result in critcial data not existing (a console error will usually alert you to this mistake).
...
Get the layer state of the layer. This state tracks the initiation life cycle. Possible values: `new`, `initiating`, `initiated`, `terminating`, `terminated`
