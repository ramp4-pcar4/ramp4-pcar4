...
* `registerHTMLConfig(html: { en: string | HTMLElement; fr: string | HTMLElement }, id: string, alertName: string, style?: PanelConfigStyle)` - Helper for `registerHTML()`. Creates and returns the `PanelConfigSet` required to register the HTML panel
