...
* `registerHTML(html: string | HTMLElement, id: string, alertName: string, style?: PanelConfigStyle, options?: PanelRegistrationOptions)` - Registers a new panel containing a screen of HTML content and returns the PanelInstance.
* `registerHTMLConfig(html: string | HTMLElement, id: string, alertName: string, style?: PanelConfigStyle)` - Helper for `registerHTML()`. Creates and returns the `PanelConfigSet` required to register the HTML panel
* `updateHTML(panel: PanelInstance, html: string | HTMLElement, id?: string)` - Updates the content of a specific HTML-based screen of a panel, using HTML content 
