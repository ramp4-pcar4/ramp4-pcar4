...
Get or set the field that is provides the content for `nameValue`. This value will be ignored if `nameArcade` is set. It is not recommended to set this to an empty or invalid string.
...
Get or set the field that is provides the content for `tooltipValue`. This value will be ignored if `tooltipArcade` is set. Set to empty string to ignore.
