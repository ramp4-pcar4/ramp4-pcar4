...
- ESRI Vector Tile Layer (`esri-tile`)
- ESRI Imagery Tile Layer (`esri-imagery-tile`)
- ESRI Image Service Layer (`esri-imagery`)
- ESRI Graphic Layer (`esri-graphic`)
- OGC WFS Web Feature Service (`ogc-wfs`)
- OGC WMS (`ogc-wms`)
...
Deriving feature primary names and maptip names act as a unit due to value defaulting. All field names are case sensitive. Missing / empty-string values will use the designated default. See the [Arcade section](../using-ramp4/layers/additional-layer-sections.md#arcade-formulas) for specifics on writing Arcade formulas for these settings.

The `nameValue(attribute)` will return the "feature name" for the given attribute, based on values of `nameArcade` or `nameField`. This is used as a default for uncustomized maptips and identify panels.
...
```js
myLayer.maptips;         // true
myLayer.maptips = false; // will no longer show maptips
```

The `maptipValue(attribute)` will return the maptip text for the given attribute, based on values of `maptipArcade` or `maptipField`. If neither are defined, `nameValue` will be used instead.

```js
myLayer.maptipValue({ other_name: 'Donna', good_name: 'Laura' }); // "Hello I'm Laura"
```

Get or set the field that provides the content for `maptipValue`. This value will be ignored if `maptipArcade` is set. Set to empty string to ignore.

```js
myLayer.maptipField; // "fun_fact"
```

Get the Arcade formula that calculates the content for `maptipValue`. An empty string indicates no formula is active.

```js
myLayer.maptipArcade; // "'This here is ' + $attr.placename"
```

Set the Arcade formula that calculates the content for `maptipValue`. An empty string will turn off any formula calculations. The method returns a Promise that resolves once the formula evaluator has been constructed.

```js
myLayer.setMaptipArcade("'Visit sunny ' + $attr.landmark"); // Promise<void>
```
