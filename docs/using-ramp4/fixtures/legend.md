...
- `itemMaxLines: number`, which determines the max number of lines a legend item can take up before truncating. Defaults to `3`. Only applies if the legend property `multilineItems` is true. Only allows integers `1`-`6` as values.
