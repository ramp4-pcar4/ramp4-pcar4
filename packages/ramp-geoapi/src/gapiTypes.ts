...
export interface IdentifyItem {
    ...
    // TODO decide if we should have a name field on this, or if consumer of identify results should be calculating that (using uid + id)
    //      names are only calculated in RAMP2 for esri formats, so probably doesn't belong on a universal interface.
    //      the same can also probably be said about symbol and id, so perhaps they should be removed and also derived by the identify consumer.
    // name: string;
    id: string; // for esri results, would be OID in string form. WMS doesn't really have id as it's return types are effectively anything it wants.
