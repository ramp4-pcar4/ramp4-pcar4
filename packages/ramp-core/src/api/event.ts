...
enum DefEH {
    ...
    OPEN_MAP_FEATURE_HOVERTIP = 'open_feature_hovertip',
    EXTENT_CHANGE_FEATURE_HOVERTIP = 'updates_feature_hovertip_extent_change'
}
    ...
    addDefaultEvents(eventHandlerNames?: Array<string>): Array<string> {
        if (
            ...
            eventHandlerNames = [
                ...
                DefEH.OPEN_MAP_FEATURE_HOVERTIP,
                DefEH.EXTENT_CHANGE_FEATURE_HOVERTIP
            ];
        }
    ...
    private defaultHandlerFactory(handlerName: string): string {
        ...
        switch (handlerName) {
            ...
            case DefEH.MAP_SCALECHANGE_SCALEBAR:
                ...
                break;
            case DefEH.OPEN_MAP_FEATURE_HOVERTIP:
                zeHandler = (mapMove: MapMove) => {
                    this.$iApi.geo.map.updateMaptipAtCoord({
                ...
                break;
            case DefEH.EXTENT_CHANGE_FEATURE_HOVERTIP:
                zeHandler = () => {
                    if (this.$iApi.geo.map.keysActive) {
                        ...
                        this.$iApi.geo.map.updateMaptipAtCoord(screenCenter);
                    } else {
                        // regular extent change, hide tooltip
                        this.$iApi.geo.map.setMaptipProperties(undefined);
                    }
                };
                this.$iApi.event.on(
