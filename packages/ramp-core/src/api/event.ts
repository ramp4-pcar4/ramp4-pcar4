...
export enum GlobalEvents {
    ...
    // fires when the user adds a layer through the wizard
    USER_LAYER_ADDED = 'user/layeradded', // Payload is FixtureInstance
            ...
            case DefEH.UPDATE_LEGEND_WIZARD_ADDED:
                zeHandler = (
                    ...
                    const legendFixture: LegendAPI =
                        this.$iApi.fixture.get('legend');
                    if (legendFixture) {
                        legendFixture.generateLegend(layer, parent);
                    }
                };
                this.$iApi.event.on(
