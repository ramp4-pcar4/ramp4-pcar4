...
describe('GeoAPI SharedUtils', () => {
    ...
    it('parseUrlIndex works', () => {
        cy.window().then(window => {
            ...
            expect(
                sharedUtils.parseUrlIndex('https://www.agr.gc.ca/atlas/rest/services/mapservices/aafc_watershed_2013/MapServer/1')
            ).to.deep.equal({
    ...
    it('parseUrlIndex works with trailing slash', () => {
        cy.window().then(window => {
            const sharedUtils = window.rInstance.geo.utils.shared;
            expect(
                sharedUtils.parseUrlIndex('https://www.agr.gc.ca/atlas/rest/services/mapservices/crop_spatial_density/MapServer/5/')
            ).to.deep.equal({
    ...
    it('parseUrlIndex no sublayer', () => {
        cy.window().then(window => {
            ...
            expect(
                sharedUtils.parseUrlIndex('https://www.agr.gc.ca/atlas/rest/services/mapservices/crop_spatial_density/MapServer')
            ).to.deep.equal({
                rootUrl: 'https://www.agr.gc.ca/atlas/rest/services/mapservices/crop_spatial_density/MapServer',
                index: 0
            });
