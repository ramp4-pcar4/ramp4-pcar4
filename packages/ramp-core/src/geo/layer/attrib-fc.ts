...
export class AttribFC extends CommonFC {
    ...
    async loadLayerMetadata(options: any = {}): Promise<void> {
        ...
        const [err, serviceResult] = await to<__esri.RequestResponse>(EsriRequest(this.serviceUrl, { query: { f: 'json' } }));
        if (!serviceResult) {
    ...
    async loadFeatureCount(): Promise<void> {
        ...
        const [err, serviceResult] = await to<__esri.RequestResponse>(EsriRequest(`${this.serviceUrl}/query`, restParam));
