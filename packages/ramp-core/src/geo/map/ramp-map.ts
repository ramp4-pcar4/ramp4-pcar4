    ...
    */
    ...
    updateAttribution(newAttribution: Attribution | undefined): void {
        ...
        if (!newAttribution || newAttribution.text.disabled) {
            ...
            let baseLayerLoadPromises: Array<Promise<__esri.Layer | null>> = [];
            const loadTimeout: number = 5000;

            // Create load promises that resolve when the baseLayer loads or after a timeout
            this.esriMap.basemap.baseLayers.forEach((bl: __esri.Layer) => {
                baseLayerLoadPromises.push(
                    new Promise((resolve, _) => {
                        let timeCreated: number = Date.now();
                        let wait = setInterval(function() {
                            if (bl.loaded && !bl.loadError) {
                                clearInterval(wait);
                                resolve(bl);
                            } else if (Date.now() - timeCreated > loadTimeout) {
                                clearInterval(wait);
                                resolve(null);
                            }
                        }, 20);
