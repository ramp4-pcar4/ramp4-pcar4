...
export class MapAPI extends CommonMapAPI {
    ...
    */
    ...
    updateAttribution(basemapConfig: RampBasemapConfig | undefined) {
        if (!basemapConfig) {
            return;
        }
        ...
        if (basemapConfig && basemapConfig.attribution) {
            // Check if attribution text is enabled
            if (basemapConfig.attribution.text.enabled) {
                attribution.text = basemapConfig.attribution.text.value;
            }

            if (basemapConfig.attribution.logo.enabled) {
                attribution.iconSrc = basemapConfig.attribution.logo.value;
                attribution.altText = basemapConfig.attribution.logo.altText;
                attribution.link = basemapConfig.attribution.logo.link;
            }
        }
