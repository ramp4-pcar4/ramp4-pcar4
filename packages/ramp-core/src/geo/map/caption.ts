...
export class MapCaptionAPI extends APIScope {
    ...
    DEFAULT_POINT_FORMATTERS: any = {
        ...
        MERCATOR: this.formatMercator,
        LAMBERT: this.formatLambert,
        UTM: this.formatUTM,
    ...
    async formatMercator(p: Point): Promise<string> {
        ...
        return `${Math.floor(projectedPoint.y)} m ${this.$iApi.$vApp.$i18n.t(
    ...
    async formatUTM(p: Point): Promise<string> {
        ...
        )} ${Math.floor(projectedPoint.y)} m${this.$iApi.$vApp.$i18n.t(
