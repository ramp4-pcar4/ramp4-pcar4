<template>
    <div
        ...
        v-tippy="{
            ...
            arrow: false,
            delay: 200,
            offset: tooltip !== undefined ? tooltip.offsetString : '0, 0'
        }"
    ></div>
</template>
...
export default class EsriMap extends Vue {
    ...
    @Get(FeatureTooltipStore.tooltip) tooltip!: any;
    ...
    onTooltipChange() {
        //@ts-ignore
        const tippy = this.$el._tippy;
        if (this.tooltip) {
            const hovertipTemplate =
                this.tooltip.template || 'Default Template';
            tippy.setContent(hovertipTemplate);
            tippy.show(0);
        } else {
            tippy.hide(0);
            tippy.setContent('');
