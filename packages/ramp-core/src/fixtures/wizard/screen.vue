...
export default defineComponent({
    ...
    methods: {
        ...
        // filter options for fields selectors
        fieldOptions(fieldName: string) {
            // for lat/long fields limit to only relevant options
            return this.layerInfo!.fields.reduce(
                (res: Array<any>, field: any) => {
                    if (
                        !fieldName ||
                        field.name.toLowerCase().includes(fieldName)
                    ) {
                        res.push({
                            value: field.name,
