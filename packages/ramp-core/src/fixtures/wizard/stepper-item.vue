<template>
    <div class="step relative flex flex-col px-12 overflow-y-hidden">
        <div class="stepper-header flex pb-24">
            <!-- step number -->
            <div v-if="!done" class="w-24 h-24 bg-gray-400 rounded-full flex justify-center items-center text-white text-xs font-semibold" :class="{ 'bg-blue-500': active }">
                {{ index + 1 }}
            </div>
            ...
            <div class="flex flex-col overflow-hidden">
                ...
                <!-- step summary -->
                <div v-show="!active" class="pl-12 text-xs truncate transition-opacity duration-1000 ease-out">
                    {{ summary }}
                </div>
            </div>
        </div>
