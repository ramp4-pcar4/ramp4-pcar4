<template>
    <div class="relative legend-header flex">
        <div class="relative">
            ...
            <tooltip class= "mx-5" v-if="!showGroup" position="right"> {{$t('toggle.group')}} </tooltip>
        </div>
        ...
        <div class="relative">
            ...
            <tooltip class="mx-5" v-if="!showVisible" position="left"> {{$t('toggle.visible')}} </tooltip>
        </div>
        <div class="absolute ml-40 top-0 left-0 bg-white shadow-md" v-if="showGroup">
            <button class="block hover:bg-gray-300 w-full p-10 text-center" @click="expand"> {{$t('group.expand')}} </button>
            <button class="block hover:bg-gray-300 w-full p-10 text-center" @click="collapse"> {{$t('group.collapse')}} </button>
        </div>
        <div class="absolute mr-32 top-0 right-0 bg-white shadow-md" v-if="showVisible">
            <button class="block hover:bg-gray-300 w-full p-10 text-center" @click="show"> {{$t('visible.show')}} </button>
            <button class="block hover:bg-gray-300 w-full p-10 text-center" @click="hide"> {{$t('visible.hide')}} </button>
        </div>   
    </div>
</template>
