...
export class LegendEntry extends LegendItem {
    ...
    toggleVisibility(
        ...
        if (this._controls.includes(Controls.Visibility)) {
            ...
            const noDefinitionsVisible: boolean = !this.layer
                ?.getLegend()
                .some(item => item.definitionVisibility);
            ...
            if (noDefinitionsVisible) {
                ...
                this._layer?.getLegend().forEach(item => {
                    item.visibility = true;
                    item.definitionVisibility = true;
                });
            }

            this.layer?.getLegend().forEach(item => {
                item.visibility = this.visibility
                    ? item.definitionVisibility
                    : false;
            });
