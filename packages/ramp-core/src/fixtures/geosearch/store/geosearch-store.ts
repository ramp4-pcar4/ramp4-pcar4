...
const mutations = {
    SET_SEARCH_RESULTS: (state: GeosearchState, searchResults: Array<any>) => {
        state.searchResults = searchResults;
    },
    SET_SAVED_RESULTS: (state: GeosearchState, saveResults: Array<any>) => {
        state.savedResults = saveResults;
    },
    SET_SEARCH_VAL: (state: GeosearchState, searchVal: string) => {
        state.searchVal = searchVal;
    },
    SET_LAST_SEARCH_VAL: (state: GeosearchState, lastSearchVal: string) => {
        state.lastSearchVal = lastSearchVal;
    },
    ...
    SET_RESULTS_VISIBLE: (state: GeosearchState, resultsVisible: boolean) => {
...
const actions = {
     ...
     *
     * @function runQuery
     * @return {Promise} promise resolving with results if found
     */
    runQuery: function(context: GeosearchContext): Promise<Array<any>> {
        // when no search value is specified
        if (!context.state.searchVal) {
            context.commit('SET_SEARCH_RESULTS', []);
            return Promise.resolve(context.state.searchResults);
        }
        ...
        if (context.state.searchVal && context.state.searchVal !== context.state.lastSearchVal) {
            return context.state.GSservice.query(`${context.state.searchVal}*`).then((data: any) => {
