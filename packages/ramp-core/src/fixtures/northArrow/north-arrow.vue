...
import HighlightLayer from 'ramp-geoapi/dist/layer/HighlightLayer';
...
export default class NorthArrowV extends Vue {
    ...
    arrow: string = `<svg xmlns="http://www.w3.org/2000/svg" fit=""  width="25px" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" focusable="false">
    ...
    mounted() {
        ...
        // don't think we should need this condition but sometimes errors without it
    ...
    async updateNorthArrow(newExtent: ApiBundle.Extent) {
        const innerShell = document.querySelector('.inner-shell') as HTMLElement;
        const arrowWidth = document.querySelector('.north-arrow')?.getBoundingClientRect().width || 0;
        ...
        if (mercator.includes(sr.wkid) || mercator.includes(sr.latestWkid)) {
            ...
            this.angle = 0;
            this.arrowLeft = (this.$iApi.map.getPixelWidth() - arrowWidth) / 2;
        } else {
            ...
            let projPole = await RAMP.geoapi.utils.proj.projectGeometry(sr, pole) as ApiBundle.Point;
            let poleScreenPos = this.$iApi.map.mapPointToScreenPoint(projPole);
            if (poleScreenPos.y < 0) {
                // draw arrow if pole not visibile
                this.displayArrow = true;
                // get angle from bottom centre 
                const bottomCentre = new ApiBundle.Point("bc", { x: newExtent.center().x, y: newExtent.ymin }, sr);
                const bc: any = await RAMP.geoapi.utils.proj.projectGeometry("EPSG:4326", bottomCentre) as ApiBundle.Point;
                ...
                this.arrowLeft = poleScreenPos.x - innerShell.offsetLeft + ((poleScreenPos.y - innerShell.offsetTop) * Math.tan(this.angle * Math.PI / 180) - arrowWidth / 2);
                // make sure arrow is within screen
                this.arrowLeft = Math.max(-arrowWidth / 2, Math.min(this.$iApi.map.getPixelWidth() - arrowWidth / 2, this.arrowLeft))
            } else {
