describe('Mapnav', () => {
    ...
    it('home button works', () => {
        cy.window().then(window => {
            cy.log(window.rInstance.geo.map.esriView);
            cy.wait(1000);

            cy.get('.mapnav .mapnav-section .w-full').eq(3).click();
            cy.wait(1000);
            cy.wrap(window.rInstance.geo.map.esriView.extent)
                .its('xmin')
            ...
            cy.wrap(window.rInstance.geo.map.esriView.extent.spatialReference)
                .its('wkid')
                .should('eq', 102100);
        });
    });
