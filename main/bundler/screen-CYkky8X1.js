...
import { t } from "./store-BUJNAlWI.js";
...
var N = { key: 0 }, P = ["content"], F = { class: "text-lg text-left flex-grow" }, I = ["innerHTML"], L = /* @__PURE__ */ e(/* @__PURE__ */ d({
	...
	setup(e) {
		...
		async function X(e) {
			...
			t === H && Y([i.data.replace(RegExp("\r", "g"), ""), b.value.map((t) => q(t.markdown, e)).filter((e) => e.trim().length > 0).join("\n\n").replace(RegExp("\r", "g"), "")].filter(Boolean).join("\n\n"), r);
		}
		return m(() => {
			P.value.push(T([
				() => r.language,
