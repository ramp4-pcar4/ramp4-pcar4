                            ...
                            fixtures: {
                                grid: {
                                    controls: [
                                        ...
                                        {
                                            actionName: 'hybridZoomAndDetails',
                                            icon: '🏆',
...
rInstance.event.on('hybridZoomAndDetails', result => {
    const layer = rInstance.geo.layer.getLayer(result.uid);

    // Only perform the zoom if the layer is not a data layer.
    if (layer.mapLayer) {
        layer.getGraphic(result.oid, { getGeom: true }).then(g => {
            rInstance.geo.map.zoomMapTo(g.geometry);
        });
    }

    // Some work to get the data structured correctly for the details panel.
    const data = result.data;
    delete data['rvInteractive'];
    delete data['rvSymbol'];
    delete data['rvUid'];

    result.params.layerCols[layer.id].forEach(attrPair => {
